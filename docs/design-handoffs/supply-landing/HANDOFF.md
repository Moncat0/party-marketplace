...
**Brand note (updated):** The product is branded **Festly** (pink `#FF2E8A`, orange `#FF6A00`, ink `#111`). Marketing uses Anton / Quicksand / Caveat; app tool chrome uses Plus Jakarta on a white canvas. Wordmark: `public/images/supply/festly-wordmark.svg` via `components/shared/BrandLogo.tsx`. Treat this handoff’s layout/copy as design intent; tokens live in `app/globals.css` and `design system/tokens/`.

## Fidelity
**High-fidelity** for layout, copy, and interaction (hover states, FAQ accordion, hero image crossfade, scroll reveals). **Not fidelity-accurate for color/type/shadow tokens** — see brand note above. Rebuild visuals against Festly's real tokens.
...
## Design Tokens
**Do not use these as final values in Festly — see brand note above.** Included only so the prototype's own styling is traceable; copies are in `tokens/*.css`.
...
Real Festly tokens to design against instead (from `app/globals.css` / `tailwind.config.js` in the repo):
...
## Assets
- `assets/festly-wordmark.svg` — placeholder wordmark logo used in nav/footer (swap for Festly's real logo).
...
- `tokens/*.css` — placeholder design-system tokens referenced by the HTML (for traceability only; do not port into Festly as-is).
