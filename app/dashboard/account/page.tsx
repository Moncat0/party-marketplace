...
export default async function AccountPage() {
  ...
  const { data: userData } = await supabase
    .from('users').select('name, first_name, last_name, phone, auth_provider, notif_marketing').eq('id', user.id).single()

  const { data: profile } = await supabase
    .from('provider_profiles').select('id').eq('user_id', user.id).single()

  if (!profile) redirect('/planner/account')

  const [{ count: pendingRequests }, { data: bookingIds }] = await Promise.all([
