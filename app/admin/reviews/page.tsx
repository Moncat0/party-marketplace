  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-[#1A1A2E]">Recensioner</h1>
        <span className="text-sm text-[#5F5E5A]">{reviews?.length ?? 0} totalt</span>
      </div>

      {!reviews || reviews.length === 0 ? (
        <p className="text-sm text-[#5F5E5A]">Inga recensioner ännu.</p>
      ) : (
            ...
            <div key={r.id} className="rounded-xl bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3 mb-1">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-[#5F5E5A] truncate">{r.users?.email ?? '—'}</p>
                </div>
              ...
              {r.comment && (
                <p className="mt-2 text-sm text-[#5F5E5A] leading-relaxed border-l-2 border-[#E8E3DC] pl-3">
                  {r.comment}
                </p>
              )}
            </div>
